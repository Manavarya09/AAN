...
from config.scrapers.configs import DUBIZZLE_CONFIG, ScraperConfig
...
class DubizzleScraper(BaseScraper):
    def __init__(self, config: ScraperConfig = DUBIZZLE_CONFIG):
    ...
    def extract_listing(self, card) -> Optional[RawListing]:
        ...
        if image_elem:
            ...
            img_bg = image_elem.get("style", "") or ""
            if "url(" in img_bg:
                import re
                urls = re.findall(r'url\(["\']?([^"\'()]+)["\']?\)', img_bg)
                raw_listing.image_urls = urls
